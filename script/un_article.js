...
{   
    var flag=0;
    var cible=localStorage.getItem("x");
    if(cible==0)
	{
		var CONTENT= document.getElementById("content");
        var cont = document.createElement("p");
        cont.className = "cont";
        cont.innerText =" Article non disponible dans notre catalogue" 
        CONTENT.appendChild(cont);
	}
	else
	{	
        for(var i=0;i<catalogue.length && flag==0;i++)
	    {   
	    	if(  cible.localeCompare(catalogue[i].nom)==0)
		    {	  
                  flag=1;
				  i--;
		    }
